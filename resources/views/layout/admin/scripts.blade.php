<script>
    ...
    function __initializePageTable(url, columns, filters = null) {
        _page_table = $('#page_table').DataTable({
            @include('layout.staff.export_buttons')
            processing: true,
            serverSide: true,
            ajax: {
                url: url,
    ...
    $.ajax({
        url: href,
        type: 'PUT', // Ensure this aligns with your backend route method
        dataType: 'json',
        success: function (result) {
            if (result.success) {
                iziToast.success({
                ...
                // Reload or update the relevant section (e.g., DataTable)
                page_table.ajax.reload(); // Ensure `page_table` is initialized
            } else {
                iziToast.error({
                    title: "Error",
                    message: result.msg,
