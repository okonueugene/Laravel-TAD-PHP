<div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="addPersonellModalLabel">Edit</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body" id="personellDetail">
            <form action="{{ url('/app/sors',[$data->id]) }}" enctype="multipart/form-data" class="form" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <input type="text" name="observation" class="form-control" id="basic-default-observation"
                        placeholder="This ...." required value="{{$data->observation}}">
                    <div class="error">
                        @error('observation')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="action_owner" class="form-control" id="basic-default-actionowner"
                        placeholder="Action Owner" required  value="{{$data->action_owner}}">
                    <div class="error">
                        @error('action_owner')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
